...
def usb_switch_progress(seconds, start_step, to_steps, progress):
    start_time = QTime.currentTime()

    if to_steps > start_step:
        direction = True
        step = int((seconds * 1000) / (to_steps - start_step))
    else:
        direction = False
        step = int((seconds * 1000) / (start_step - to_steps))

    while start_time.msecsTo(QTime.currentTime()) < seconds * 1000:
        if start_time.msecsTo(QTime.currentTime()) % step == 0:
            set_value = int(start_time.msecsTo(QTime.currentTime()) / step)
            if direction:
                set_value = start_step + set_value
                progress.setValue(set_value)
            else:
                set_value = to_steps - set_value
                progress.setValue(set_value)
        QEventLoop().processEvents()
    progress.setValue(to_steps)
...
class MyMainWindow(QMainWindow, main_ui.Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.usb_switch_dialog = MyUsbSwitchDialog()
        ...
        self.usb_switch_dialog.setWindowIcon(load_icon("swap"))
        ...
        self.actionUSB_Switch_Set.triggered.connect(self.usb_switch_func)
        self.usb_switch_dialog.radioButton_master.clicked.connect(
            lambda: self.usb_switch_func(1)
        )
        self.usb_switch_dialog.radioButton_float.clicked.connect(
            lambda: self.usb_switch_func(2)
        )
        self.usb_switch_dialog.radioButton_controlled.clicked.connect(
            lambda: self.usb_switch_func(3)
        )
        self.actionUSB_Auto_Switch.triggered.connect(self.usb_auto_switch)
    ...
    def usb_switch_func(self, s):
        if s == 1:
            ...
            if reply[0] == 0x6F and reply[2] == 3:
                ...
                QMessageBox.critical(self, "Error", "Function reply error", QMessageBox.Ok)

            wm_pos = self.geometry()
            wm_size = self.size()
            self.usb_switch_dialog.move(
                (wm_pos.x() + wm_size.width() / 2 - self.usb_switch_dialog.width() / 2) - 20,
                (wm_pos.y() + wm_size.height() / 2 - self.usb_switch_dialog.height() / 2) - 80,
            )
    ...
    def usb_auto_switch(self):
        usb_switch_status_value = 0
        progress_setLabelText = ""

        # Create a progress bar window
        progress = QProgressDialog(self)
        progress.setWindowModality(Qt.ApplicationModal)
        progress.setWindowTitle("Loading...")
        progress.setLabelText("等待设备...")
        ...
        if reply[0] == 0x6F and reply[2] == 3:
            ...
            progress.close()
            QMessageBox.critical(self, "Error", "Function reply error", QMessageBox.Ok)
        progress.setWindowTitle(progress_setLabelText)
        ...
        # The suspended state defaults to the main USB device
        if usb_switch_status_value == 0:  # float
            payload[2] = 1
        # Switch to another USB device
        elif usb_switch_status_value == 1:  # master
            payload[2] = 2
        elif usb_switch_status_value == 2:  # device
            payload[2] = 1
        else:
            # logger.debug("usb_switch unknown error")
            return
